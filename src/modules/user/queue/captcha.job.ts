...
export class CaptchaJob {
    ...
    async send(params: SendParams): Promise<{ result: boolean; log: any }> {
        const { data, action, type, code } = params;
        let log: any;
        const result = true;
        ...
        try {
            ...
            console.log('=== 验证码生成调试信息 ===');
            console.log('生成的验证码:', captchaCode);
            console.log('收件人:', (data as any).email || (data as any).phone);
            console.log('操作类型:', action);
            console.log('验证码类型:', type);

            // 创建验证码模型实例
            const captcha = await this.createCaptcha(data, action, type, time, captchaCode);
            const expired = 1800; // 硬编码过期时间
            const otherVars =
            ...
            console.log('验证码已保存到数据库，准备加入队列:', jobName);

            // 加入异步发送任务
            const job = await this.captchaQueue.add(jobName, {
                captcha: instanceToPlain(captcha),
                option: { ...config, ...time },
                otherVars,
            });

            console.log('验证码任务已加入队列, 任务ID:', job.id);
            console.log('队列状态:', await this.captchaQueue.getJobCounts());
        } catch (err) {
