...
import { IsDefined, IsEnum, IsNotEmpty, IsOptional, IsUUID, MaxLength, ValidateIf } from 'class-validator';
...
export class CreateCommentDto {
    ...
    @IsDefined({ message: '关联内容ID必须指定' })
    targetId!: string;

    @ApiProperty({
        description: '评论所属文章ID（当category为post时必填）',
    })
    ...
    @ValidateIf((value) => value.category === 'post')
    @IsOptional()
    post?: string;
